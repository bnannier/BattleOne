...
// ----------------------------------------------------------------------------------------------------
// Mouse definitions and Joystick definitions
// ----------------------------------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------------------------------
// Class Input, Handles all Direct input related opperations
// ----------------------------------------------------------------------------------------------------
class Input{
protected:
	HWND			i_hWnd;
	IDirectInput8  *i_DI;
public:
	Input(){i_DI = NULL;}								// Clear the DirectInput interface pointer
    ~Input(){Shutdown();}								// Force a shutdown

	BOOL			Shutdown();							// Release the COM objects i_DI
	BOOL			Init(HWND hWnd, HINSTANCE hInst);	// Create a DirectInput interface

	HWND			GethWnd(){return i_hWnd;}			// Return the parent window handle
	IDirectInput8  *GetDirectInputCOM(){return i_DI;}	// Return a pointer to IDirectInput8 object
};


// ----------------------------------------------------------------------------------------------------
// Class InputDevice, Handles all InputDevice related opperations (Keyboard, Mouse)
// ----------------------------------------------------------------------------------------------------
class InputDevice{
public:
	IDirectInputDevice8	   *i_DIDevice;
	Input				   *i_Input;
	short					i_Type;
	BOOL					i_Windowed;

	char					i_State[256];
	DIMOUSESTATE		   *i_MouseState;
...
public:
   ...
   ~InputDevice(){Free();}
	...
	IDirectInputDevice8	   *DeviceCOM(){return i_DIDevice;}							// Return the pointer to the IDirectDevice8 object
	BOOL					Create(Input *input, short Type, BOOL Windowed = TRUE); // Creates and initializes an instance of a device based on a given globally unique identifier (GUID)
	BOOL					Free();													// Releases access to the device
	BOOL					Clear();												// Clear all vales in this class
	BOOL					Read();													// Retrieves immediate data from the device
	BOOL					Acquire(BOOL Active = TRUE);							// Obtains access to the input device

	LPDIRECTINPUTEFFECT		GetEffects(){return g_lpdiEffect;}

	//// Keyboard specific functions
	BOOL					GetKeyState(char Num);									// Check if key is pressed
	BOOL					SetKeyState(char Num, BOOL State){						// Set a new value for key press
								i_State[Num] = State;
                                return TRUE;
							}

	// Mouse specific functions
	long					GetXDelta();											// Position of mouse on the x-axis
	long					GetYDelta();											// Position of mouse on the y-axis
	long					GetXPos();
	long					GetYPos();
	BOOL					GetButtonState(char Num);								// Button pressed on the mouse
};
#endif